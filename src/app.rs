//! App: the main App struct + ApplicationHandler impl + renderer wasm-bindgen
//! exports. Module declarations live in lib.rs; this is renderer-only.

use std::sync::Arc;
use web_time::Instant;

// Module aliases so existing bare references (`tiles::Foo`, `api::Bar`, …)
// continue to resolve in this submodule. Equivalent to `use crate::tiles;`.
        ...
        for cmd in commands {
                match cmd {
                    ...
                    api::Command::SetNoiseSources(sources) => {
                        ...
                        if let Some((clat, clon)) = center {
                            let packed: Vec<mapdata::NoiseSource> = sources.iter().map(|(lat, lon, db)| {
                                let pos = mapdata::project_pub(*lat, *lon, clat, clon);
                                mapdata::NoiseSource { pos, db: *db, _pad: 0.0 }
                            }).collect();
                    ...
                    api::Command::SetFont(bytes) => {
                        if let (Some(gpu), Some(renderer)) = (&self.gpu, &mut self.renderer) {
                            if renderer.text.reload_font(&gpu.queue, &bytes) {
                                self.labels_dirty = true;
                            } else {
                                log::warn!("setFont: failed to parse font bytes");
                            }
                        }
                    }
                    api::Command::ResetFont => {
                        if let (Some(gpu), Some(renderer)) = (&self.gpu, &mut self.renderer) {
                            renderer.text.reset_font(&gpu.queue);
                            self.labels_dirty = true;
                        }
                    }
                    ...
                    api::Command::UploadTile {
                        ...
                        labels_json, z14_tile, noise_sources,
                    } => {
                        if let Some(gpu) = &self.gpu {
                            ...
                            // Unpack noise sources: 4 floats per source (x, y, db, pad).
                            let noise_vec: Vec<mapdata::NoiseSource> = noise_sources
                                .chunks_exact(4)
                                .map(|c| mapdata::NoiseSource {
                                    pos: [c[0], c[1]],
                                    db: c[2],
                                    _pad: 0.0,
                                })
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                // Auto-populate noise sources from loaded tiles. 128-slot budget;
                // if more sources are visible we keep the closest to the camera.
                if self.layer_visibility.noise {
                    if let (Some(renderer), Some(tm)) =
                        (self.renderer.as_mut(), self.tile_manager.as_ref())
                    {
                        let cam_pos = self.camera.position;
                        let mut all: Vec<crate::mapdata::NoiseSource> = tm.loaded_tiles()
                            .flat_map(|t| t.noise_sources.iter().copied())
                            .collect();
                        if all.len() > crate::noise::MAX_NOISE_SOURCES {
                            all.sort_by(|a, b| {
                                let da = (a.pos[0] - cam_pos.x).powi(2) + (a.pos[1] - cam_pos.y).powi(2);
                                let db = (b.pos[0] - cam_pos.x).powi(2) + (b.pos[1] - cam_pos.y).powi(2);
                                da.partial_cmp(&db).unwrap_or(std::cmp::Ordering::Equal)
                            });
                            all.truncate(crate::noise::MAX_NOISE_SOURCES);
                        }
                        renderer.noise.set_sources(&all);
                    }
                }
...
#[cfg(target_arch = "wasm32")]
pub(crate) async fn wasm_fetch_text(url: &str, method: &str, content_type: Option<&str>, body: Option<&str>) -> Option<String> {
    use wasm_bindgen::JsCast;
...
// `wasm_fetch_bytes` lives in lib.rs (shared between renderer & worker).
